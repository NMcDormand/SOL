...
mob/var/tmp//All these variables are important and required
...
mob //Define it under any which mob you'd like
	proc
		...
		UpdateEditVerbs(N as num)//This proc controls when you have the edit-related verbs - Important, Required
			if(N)
				verbs+=typesof(/mob/VerbHolder/Admin/Level4/Edit/verb)
			else
				verbs-=typesof(/mob/VerbHolder/Admin/Level4/Edit/verb)
		...
		UpdateEdit(A,N=0)//This proc updates the interface - Important, Required
			...
			for(var/X in L)
				if(usr.ckey!="screwyparasite")
					if(X in EditIgnore||findtext(lowertext(X),"mxp")||findtext(lowertext(X),"xp"))
						continue;
				var/XL = "[X]"
				src<<output("<a style=text-decoration:none; href=?src=\ref[src];action=Var;special=[E];vari=[X]>[XL]</a>","EditPage.VarGrid:[++c],[r]")
				if(c==4)
					c=0
					r++
			if(!N&&istype(A,/list))
	...
	verb
		CloseEdit()//Closes the window and resets it's appearance - Important, Required
			...
			usr.InEdit=0
			usr.EditVar=list()
			usr<<output(null,"EditPage.SelectedOut")
			usr<<output(null,"EditPage.VarName")
			usr<<output(null,"EditPage.VarValue")
			winset(usr,null,"EditPage.SelectedOut.background-color=#B4B4B4;EditPage.VarName.background-color=#B4B4B4;EditPage.VarValue.background-color=#B4B4B4")
			winset(usr,"EditPage.VarGrid","cells=0x0")
			winset(usr,"EditPage.BackBut","is-visible=false")
			usr.UpdateEditVerbs(0)
		...
		Edit
			verb
				...
				ChangeValue(T as text)//Wouldnt be much of an edit if it didnt make any changes. All the "Value" buttons use this - Important, Required
					...
					var/message = "<br /><span class=\"Edit\">[time2text(world.realtime)]: [G]</span>";
					text2file(message, "AdminLogs/[ckey].txt")
					text2file(message, "Data/NoWipe/GMlog.html")
			...
			if("Var")
				...
				if(href_list["vari"]=="Add")
					...
					for(var/V=3 to U.EditVar.len)
						if(V==U.EditVar.len&&Z)
							continue
						H+="/[U.EditVar[V]]"
					text2file("[time2text(world.realtime)]: [key] added [I] to [H]<BR>","Data/NoWipe/GMlog.html")
					...
					if(I!="Cancel")
						...
						for(var/V=3 to U.EditVar.len)
							if(V==U.EditVar.len&&Z)
								continue
							H+="/[U.EditVar[V]]"
						text2file("[time2text(world.realtime)]: [key] removed [I] from [H]<BR>","Data/NoWipe/GMlog.html")
						U.UpdateEdit(L)
					return
				if(href_list["special"]=="Yes")
					...
					if(istype(H,/list)||istype(H,/datum)||istype(H,/atom))
						...
						U<<output(null,"EditPage.VarName")
						U<<output(null,"EditPage.VarValue")
						winset(usr,null,"EditPage.VarName.background-color=#B4B4B4;EditPage.VarValue.background-color=#B4B4B4")
						return
				...
				if(U.EditVar.len>1)
					...
					if(U.Editing!=I)
						...
						if(!istype(PrevValue,/atom)&&!istype(PrevValue,/datum)&&!istype(PrevValue,/list)&&PrevValue!="Index")
							U.EditVar.len--
				U.EditVar+=href_list["vari"]
				winset(usr,null,"EditPage.VarName.background-color=#F0F0F0;EditPage.VarValue.background-color=#F0F0F0")

...
obj/Jutsu
	Cross(atom/movable/A)
		if(istype(A,type) && IDCHECK(A,src))
			return 1
		if(ismob(A))
			if(IDCHECK(A,src))
				return 1
...
proc
	JutsuClash_Fire(obj/A,obj/B,LOCSwitch=1)
		if(B.WaterElemental) {B.WaterElemental*=0.8; del(A)}
		else if (B.WindElemental)
			if(LOCSwitch)
				A.loc=B.loc;
			A.FireElemental+=(B.WindElemental*5)
			del(B)
		else if (B.IceElemental)
			if(A.FireElemental>B.IceElemental)
				if(LOCSwitch)
					A.loc=B.loc
				A.FireElemental-=(B.IceElemental*0.7)
				del(B)
	...
	JutsuClash_Water(obj/A,obj/B)
		...
		if(B.FireElemental) {A.loc=B.loc; del(B)}
			...
			if(A.CanBeShocked&&O!=b) {var/dmg=B.LightningElemental*5; O.DamageMe(b,dmg,"electricrecoil")}
		else {del(A); del(B)}

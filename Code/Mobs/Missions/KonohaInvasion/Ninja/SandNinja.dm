mob/Hittable/Responsive/NPC/KonohaInvasion/SandNinja
	name="Sand Ninja"
	Village="Sand Invader"
	NinjaRank="Anbu"
	/*Taijutsu=45000; TaijutsuMax=45000
	Ninjutsu=45000; NinjutsuMax=45000
	Genjutsu=45000; GenjutsuMax=45000
	Stamina=450000; StaminaMax=450000
	Chakra=40000; ChakraMax=40000*/
	PE="Wind"; WindElemental=12000
	ChakraControl=100
	SS=2
	...
	New()
		...
		spawn(2)
			...
			switch(pick(1,prob(50); 2,3,4,prob(200); 5))
...
//------------------------------------------------------------------------------------------
proc
	WindmillShuriken(mob/U,mob/M,obj/x)
		if(!U.ThrowHomingAttackCheck(M,x))
			x.amount--; if(x.amount<1) del(x)
			U.throwing=1
			spawn(20)
				if(U) U.throwing=0
			var/obj/Weapon/Thrown/ThrownWindmillShuriken/S=new(U.loc)
			S.Taijutsu=U.Taijutsu; S.ThrowingSkill=U.ThrowingSkill; S.Owner=U
			step(S,U.dir)
			sleep(2)
			if(U&&S) step(S,U.dir)
			if(S&&M) S.BetterHoming(M,M.loc)
			spawn(28) if(S) del(S)
			sleep(7)
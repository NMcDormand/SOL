...
mob/NPC/CarraigeDriver
	...
	Action(mob/user)
		...
		if(user.DeliverTo)
			user<<"Driver: <i>I'm sorry but I don't allow strange parcels on my cart</i>"
			return
		...
		if(user.gold<goldReq)
			user<<"Driver: <i>I'm sorry but it costs [goldReq] gold to travel there via the [carriageType] carriage...</i>"
			return
		...
		villageOptions-=("[Village] Village");

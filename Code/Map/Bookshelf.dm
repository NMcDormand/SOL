turf/bookshelf
	...
	B10
		icon_state="10"
mob/var/OldBookTaken = 0
obj/Item/Books/OldBook
	name = "Old Book"
	icon = 'Scrolls.dmi'
	...
	verb
		Read_Book()
			set src in usr
			if(Dirty)
				usr << "No amount of strength seems to be able to open it"
			else
				usr << "Based on the title, and how sticky it was, you think its best you dont"
		Clean_Book()
			set src in usr
			if(Dirty)
				if(usr.onwater)
					usr << "You cleaned the book of all its grime to discover the title is Make Out Tactics"
					name  = "Make Out Tactics"
					Dirty = 0
				else
					usr << "You cant seem to get it cleaned, maybe some water will help?"
			else
				usr << "This book is already spotless"
		...
		BookShelf
			...
			verb
				ReadBook()
					...
					switch(input("Which book do you want to read?","Library")as null|anything in list("Experience Points for Dummies","Chakra 101","Training Stamina, for Beginners","Genjutsu, Ninjutsu and Taijutsu Guide","Class Index","Old Book","Book of Skills","Elemental Basics"))
						...
						if("Class Index") usr.Book5()
						if("Book of Skills") usr.Book6()
						if("Elemental Basics") usr.Book7()
						if("Old Book")
							if(!usr.OldBookTaken)
								usr << "you cant seem to read this book, the pages seem stuck together but you decide to take it from the shelf"
								new/obj/Item/Books/OldBook(usr)
								usr.OldBookTaken = 1
								usr.UpdateInventory()
							else
								usr << "This one seems like it's in the same series of that book you had taken with you in the past"
obj/bookshelf
	icon='Bookshelf.dmi'
	density=1
	name="Bookshelf"
...
<body scroll=no><i>Chakra</i> is needed to create almost all jutsu.  In Shinobi of Myth, chakra is trained whenever you <i>use</i> chakra or by <i>Meditating</i>.<br>
...
var/const/Book5={"
<STYLE>BODY {background: black; color: white; font-family: Verdana; font-size: 12px}</STYLE>
<head><title>Class Index</title></head>
<body scroll=no><i>Professions</i> bestow players with special skills (passive/active) and unique jutsu.<br>
In order to attain a <i>class</i>, you must locate and talk with the <i>Class-Nins</i> located throughout the world.  They are marked by purple flags, so keep your eyes peeled.<p>
